...
export function Header() {
  ...
  return (
    ...
    >
      <nav className="glass gloss flex w-full max-w-5xl items-center gap-2 rounded-full px-3 py-2">
        <a href="#top" className="ml-2 flex items-center gap-2 pr-2 font-display text-sm font-semibold tracking-tight">
          <span className="animate-pulse-node inline-block h-2 w-2 rounded-full bg-primary" />
        ...
        <div className="mx-auto hidden items-center gap-1 md:flex">
          {LINKS.map((l) => (
            <a
              key={l.href}
              href={l.href}
              className={`relative z-10 rounded-full px-3.5 py-1.5 text-sm transition-all duration-300 ${
                active === l.href
                  ? "glass-soft font-medium text-foreground"
          ...
          <a
            ...
            rel="noreferrer"
            aria-label="GitHub"
            className="glass-soft rounded-full p-2 text-muted-foreground transition-all hover:text-primary"
          >
        ...
        >
          {LINKS.map((l) => (
            <a
              ...
              className="block rounded-2xl px-4 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-primary/8 hover:text-foreground"
            >
              {l.label}
            </a>
          ))}
